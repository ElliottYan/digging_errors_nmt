        ...
        try:
            ...
            with torch.no_grad():
                if trgt_sentences:
                    ...
                    #     hypos = []
                    #     logging.warn('| Warning: Out of memory! Trying to empty cache.')
                    #     if getattr(decoder, "batch_size", 1) > 1:
                    #         decoder.batch_size = int(decoder.batch_size * 0.5)
                    #         logging.info('| INFO: Reset batch size to {}!'.format(decoder.batch_size))
                    #     import gc
                    #     gc.collect()
                    #     torch.cuda.empty_cache()
                    #     torch.cuda.memory_stats()
                    #     decoder.decode(src, retry=True)
