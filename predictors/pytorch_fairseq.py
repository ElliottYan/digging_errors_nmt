...
class FairseqPredictor(Predictor):
    ...
    @torch.no_grad()  
    def batch_predict_next(self, inputs, incremental_states):
        """Call the fairseq model."""
        if not isinstance(inputs, torch.Tensor):
            inputs = [torch.LongTensor(each_input) for each_input in inputs]
            inputs = torch.nn.utils.rnn.pad_sequence(inputs, batch_first=True, padding_value=self.pad_id)
        
        if self.use_cuda:
            inputs = inputs.cuda()

        self.encoder_outs = self.reorder_encoder_out(self.encoder_outs, torch.zeros(len(inputs), dtype=torch.int64, device=inputs.device))
        lprobs, _  = self.model.forward_decoder(
            inputs, self.encoder_outs, incremental_states)
        lprobs[:, self.pad_id] = utils.NEG_INF
        return lprobs

    def reorder_incremental_states(self, incremental_states, new_order):
        return self.model.reorder_incremental_state(incremental_states, new_order)
    
    def reorder_encoder_out(self, encoder_out, new_order):
        return self.model.reorder_encoder_out(encoder_out, new_order)
