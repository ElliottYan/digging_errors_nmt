...
def main(args):
    ...
    ret = compute_exact_match(dfs_outputs, beam_outputs)
    print("Exact match err is {}".format(ret))


def compute_exact_match(dfs_outputs, beam_outputs, only_top1=True):
    if only_top1 is False:
    ...
    for key in dfs_outputs:
        if key not in beam_outputs:
            continue
        cnt += 1
        dfs_text = get_text(dfs_outputs[key][0][1])
        beam_text = get_text(beam_outputs[key][0][1])
        # only care about top1
