...
def main(args):
    ...
    ret = compute_inversions(dfstopk_scores)
    
    print("Computed input directory is {}.".format(dfs_path))
    print("Model errors with average oracle ranks is {}.".format(ret))
...
def compute_inversions(all_scores):
    ...
    for cur_scores in all_scores:
        ...
        for i in range(len(cur_scores)):
            if oracle_score < cur_scores[i]:
                oracle_score = cur_scores[i]
                oracle_rank = i
        oracle_ranks.append(oracle_rank)
    return mean(oracle_ranks)
