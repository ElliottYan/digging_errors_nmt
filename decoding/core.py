...
class Hypothesis:
    ...
    def strings(self):
        return "%s" % ' '.join(str(w) for w in self.trgt_sentence)
...
class PartialHypothesis(object):
    ...
    def _new_partial_hypo(self, states, word, score, base_score=None, breakdown=None, cur_max=None):
        ...
        if self.statistics is not None:
            ...
            new_hypo.statistics.push(new_hypo.score_breakdown[-1] if new_hypo.score_breakdown else 0.0, cur_max)
        
        return new_hypo

    def expand(self, word, new_states, score, score_breakdown=None, base_score=None, cur_max=None):
        return self._new_partial_hypo(new_states, word, score, base_score=base_score, breakdown=score_breakdown, cur_max=cur_max)
...
class Decoder(object):    
    ...
    def apply_predictor(self, hypo=None, top_n=0, disable_combine=False):
    ...
    def initialize_predictor(self, src_sentence, retry=False):
        ...
        if self.max_len_factor != -1:
            self.max_len = int(np.ceil(self.max_len_factor * len(src_sentence)))
        else:
            self.max_len = INF
        self.full_hypos = []
        self.current_sen_id = self.current_sen_id + 1 if retry is False else self.current_sen_id
    ...
    @abstractmethod
    def decode(self, src_sentence, tgt_sentence=None):
